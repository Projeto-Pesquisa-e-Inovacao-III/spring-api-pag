...
public record Item(
    ...
    private static void validateExternalItemId(String externalItemId){
        if(externalItemId == null || externalItemId.trim().isEmpty()){
            throw new InvalidFieldException("externalItemId", "deve ser não nulo ou vazio");
        }
    }

    private static void validateName(String name) {
        if (name == null || name.trim().isEmpty())
            throw new InvalidFieldException("name", "deve ser não nulo e não vazio");
        if (name.length() > 100)
            throw new InvalidFieldException("name", "deve ter no máximo 100 caracteres");
    }

    private static void validateDescription(String description) {
        if (description != null && description.length() > 255)
            throw new InvalidFieldException("description", "deve ter no máximo 255 caracteres");
    }

    private static void validateQuantity(Integer quantity) {
        if (quantity == null || quantity < 1 || quantity > 999)
            throw new InvalidFieldException("quantity", "deve ser não nula, maior ou igual a 1 e menor ou igual a 999");
    }

    private static void validateUnitAmount(Integer unitAmount) {
        if (unitAmount == null || unitAmount < 0 || unitAmount > 999_999_900)
            throw new InvalidFieldException("unitAmount", "deve ser não nulo, no mínimo 0 e menor ou igual que 999999900");
    }

    private static void validateImageUrl(String imageUrl) {
        if (imageUrl != null && imageUrl.trim().isEmpty())
            throw new InvalidFieldException("imageUrl", "não pode ser vazia se fornecida");
    }
}
