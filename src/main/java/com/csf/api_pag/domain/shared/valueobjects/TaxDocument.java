...
import com.csf.api_pag.domain.checkout.exception.CnpjIsNotSupportedException;

import java.util.regex.Pattern;

public class TaxDocument {

    private static final Pattern ALL_DIGITS_REPEATED_PATTERN = Pattern.compile("(\\d)\\1{10}");
    ...
    private static void validateValue(String value, TaxDocumentType type) {
        if (type.equals(TaxDocumentType.CPF)) {
            if (!isCpfValid(value)) {
                throw new InvalidFieldException("taxDocument.value", "CPF inválido: " + value);
            }
        }
        if (type.equals(TaxDocumentType.CNPJ)) {
            throw new CnpjIsNotSupportedException();
    ...
    private static boolean isCpfValid(String cpf) {
        ...
        boolean allDigitsRepeated = ALL_DIGITS_REPEATED_PATTERN.matcher(cleaned).matches();
    ...
    private static String clearCpf(String cpf) {
        return NON_DIGIT_PATTERN.matcher(cpf).replaceAll("");
    }
}
